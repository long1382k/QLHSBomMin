...
{
    public partial class frToanBoNhanVien : Office2007Form
    {
        ...
        public frToanBoNhanVien(SuperTabControl spTab)
        {
            InitializeComponent();
            ...
            //add menu con
            itemSuaHS = conMenu.Items.Add("Sửa thông tin nhân viên");
            itemXoaHS = conMenu.Items.Add("Xóa thông tin nhân viên");
        ...
        private void btnSuaHoSo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                A_ThemNhanSu them = new A_ThemNhanSu(Option.sua, idNhanSu,0);
                them.ShowDialog();
                LoadData();
        ...
        private void btnXoaHoSo_Click(object sender, EventArgs e)
        {
            ...
            {
                idNhanSu = Convert.ToInt32(this.dgv_nhansu.CurrentRow.Cells["MaNhanVien"].Value);
                if (MessageBox.Show("Bạn có chắc chắc muốn xóa nhân viên?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
            ...
            {
                MessageBox.Show("Lỗi khi xóa nhân sự");
            }
        }
        ...
        private void btnThemHoSo_Click(object sender, EventArgs e)
        {
            try
            {
                A_ThemNhanSu them = new A_ThemNhanSu(Option.them, 0,0);
                them.ShowDialog();
                LoadData();
